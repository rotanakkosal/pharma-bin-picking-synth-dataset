...
| `v1.1-realistic-labels` (2026-05-12) | **Label pool overhaul + UV band fix.** Replaced 37 fake-Korean procedural labels with 30 ChatGPT-generated realistic Korean pharmacy labels (syrup/tablet/capsule/powder/injection, varied colors and layouts). Quarantined 5 off-domain external pharma-graphic labels to `textures/labels_distractors/`. Active pool now 32 (30 ChatGPT + 2 photoreal). `make_label_material`: added V band [0.10, 0.80] (cap + base show bare HDPE) and U band [0.15, 0.85] (back of bottle bare, hides UV seam) — bottles now look like real pharmacy products with white caps and a label band. **Current. Synth re-marked terminal at this version.** |
| `eval-v2` (2026-05-12, **eval-only — dataset unchanged**) | Overhauled `scripts/eval/eval_uoais_on_synth.py` after two adversarial reviews — see [`docs/eval/eval_methodology.md`](docs/eval/eval_methodology.md). Switched to one Hungarian (optimal) match per scene on **visible** masks (was greedy on amodal); added F@.75 (Dice) + per-image Overlap-P/R/F (the published-UOAIS comparators); added occlusion-stratified recall + a frame-truncated row; added a small-mask filter (default ≥100 visible px, swept 50/100/500 — not load-bearing); kept the old greedy-amodal metric as a continuity check. **The headline F1 moved 0.832 → 0.845 because of the protocol change, not the model or the GT** — the v1.1 render is byte-identical. Under the new protocol: precision 0.917 / recall 0.784 / F1 0.845 (legacy metric still reproduces 0.910 / 0.766 / 0.832 exactly). Stratified recall: ≤30%-occluded ("pickable") = 0.94; 30–50% = 0.69 (the cliff); ≥80% = 0.00 (effectively unobservable — the aggregate 0.77 is dominated by these and should not be read as detection ability on pickable bottles). F@.75(visible, Dice) = 0.75 vs UOAIS-Net's ≈0.79 on real OSD-Amodal — our scenes are denser than OSD, so the gap is density, not difficulty (the *opposite* of the old "too easy" read). One nameable clean failure: plain-white-bottle-on-plain-white-background separation (blue_cap_pill_bottle recall 0.50, white_pill_bottle 0.57; the colourful syrups are 0.83–0.92). |
