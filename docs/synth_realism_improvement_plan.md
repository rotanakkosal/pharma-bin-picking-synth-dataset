...
- **Eval impact:** under the *old* eval protocol (greedy amodal-match), UOAIS on the 30-scene v1.1 batch scored F1 0.832 / IoU 0.857 vs v1.0-final 0.828 / 0.853 — essentially unchanged, expected (UOAIS segments on geometry/depth, not label text). The realism gain is visual/domain-fidelity, not difficulty.
- **Eval protocol overhaul (2026-05-12, eval-only — render unchanged):** after two adversarial reviews, `scripts/eval/eval_uoais_on_synth.py` was rewritten — see [`eval/eval_methodology.md`](eval/eval_methodology.md). One Hungarian match per scene on visible masks (was greedy on amodal); added F@.75 (Dice) + per-image Overlap-P/R/F (published comparators); occlusion-stratified recall + a frame-truncated row; a small-mask filter (≥100px default, swept — not load-bearing); old greedy-amodal metric kept as a continuity check. **The headline F1 moved 0.832 → 0.845 because of the protocol change, not the model or GT.** New protocol: precision 0.917 / recall 0.784 / F1 0.845 (legacy metric still reproduces 0.910/0.766/0.832). Stratified: ≤30%-occluded recall 0.94; 30–50% 0.69 (the cliff); ≥80% 0.00 (unobservable — the aggregate 0.77 is dominated by these). F@.75(visible,Dice) 0.75 vs UOAIS-Net ≈0.79 on OSD — our scenes denser, so gap = density not difficulty (opposite of the old "too easy" read).
- **Outlier classes:** blue_cap_pill_bottle (recall 0.50) and white_pill_bottle (0.57) — *not* a small-object/occlusion artifact as previously hypothesised. The real cause: plain-white-bottle-on-plain-white-background separation (low RGB contrast); the featureless white classes suffer, the colourful syrups (0.83–0.92) don't. Next step is the depth-channel diagnostic in `eval/eval_methodology.md` (is `normalize_depth`'s clip range squashing the inter-bottle gradient?) — *not* QuBER (mask refinement can't recover undetected/merged bottles).
